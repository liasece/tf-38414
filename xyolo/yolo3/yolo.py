...
from xyolo.yolo3.utils import load_tfrecord_dataset,load_tfrecord_dataset_xyolo,get_random_data_xyolo
import time
...
class YOLO(object):
    ...
    def __init__(self, config=None, train=False, **kwargs):
        ...
        self.tf_train_path = config.tf_train_path
        self.tf_val_path = config.tf_val_path
    ...
    def fit(self, **kwargs):
        ...
        callbacks = []
        if use_tensorboard:
            logging = TensorBoard(log_dir=tensorboard_log_path, histogram_freq=1, profile_batch=(5,10))
        ...
        if frozen_train:
            ...
            train_data = load_tfrecord_dataset(self.tf_train_path, self.model_image_size, batch_size, self.anchors, num_classes)
            val_data = load_tfrecord_dataset(self.tf_val_path, self.model_image_size, batch_size, self.anchors, num_classes)
            logger.info(
